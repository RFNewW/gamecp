...
if ($username != '' && $password != '') {
    ...
    if (!preg_match(REGEX_USERNAME, $username) || !preg_match(REGEX_PASSWORD, $password)) {
        $out .= "<div style='text-align: center;'>" . _l('Invalid Username or Password. Please try again.') . "</div>";
        $exit_stage1 = true;
    }
...
if ($exit_stage1 != true) {
    ...
    if (mssql_num_rows($query_user) <= 0) {
        $out .= "<span style='text-align: center'>" . _l('Invalid Username or Password. Please try again.') . "</span>";
    } else {
        ...
        if ($username == $query_username && $password == $query_password) {
            ...
            if ($userdata['serial'] == "") {
                ...
                session_destroy();
                $out .= '<p style="text-align: center; font-weight: bold;">' . _l('You must log in the game (via the launcher) at least once in order to use the Game CP.') . '</p>';
            }

            @mssql_free_result($query_account);
            @mssql_free_result($query_ban);
        } else {
            $out .= "<center>" . _l('Invalid Username or Password. Please try again.') . "</center>";
            $exit_stage2 = true;
        }
