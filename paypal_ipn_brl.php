...
if (!($user_info_result = mssql_query($user_info_sql))) {
    ...
    gamecp_log(5, $custom, "PAYPAL - ERRO - Não é possível encontrar ou consultar esse ID de usuário");
    exit;
}
...
if ($user['AccountName'] != '') {
    ...
    $user_name = $custom;
    gamecp_log(5, $custom, "PAYPAL - ERRO - Não foi possível procurar a conta fornecida pelo PayPal: $custom");
    gamecp_log(5, $custom, "PAYPAL - ERROR - Não creditou ID TXN: $txn_id | Payer ID: $payer_id | Payer Email: $payer_email");
    exit;
}
...
if (!$fp) {
    ...
    $log_message = "PAYPAL - CARREGADO PAYPAL.COM";
    ...
    if ($res_status == 'verified') {
        ...
        $credits = calculate_credits($config['donations_credit_muntiplier'], $config['donations_number_of_pay_options'], $config['donations_start_price_brl'], $config['donations_start_credits_brl'], $payment_gross);
        ...
        if ($payment_status == 'Completed') {
            ...
            if ($business_email == $config['paypal_email']) {
                ...
                if (mssql_num_rows($tnx_query) == 0) {
                    ...
                    // Always log this in the admin logs!
                    gamecp_log(0, $user_name, "PAYPAL - PAGAMENTO BEM SUCEDIDO - ID TXN: $txn_id | Amount: $" . $payment_gross);
                    ...
                    if (mssql_num_rows($totalusers_query) == 0) {
                        ...
                        // Always log this in the admin logs!
                        gamecp_log(0, $user_name, "CRÉDITOS PAYPAL - ADICIONADOS - INSERIR - ID TXN: $txn_id | Credits: $credits");
                    } else {
                        ...
                        // Always log this in the admin logs!
                        gamecp_log(0, $user_name, "PAYPAL - CRÉDITOS ADICIONADOS - ATUALIZAÇÃO - ID TXN: $txn_id | Credits: $credits");
                    }
                ...
                // Recommend this log as 'highly suspecious', level 5 log
                $log_message = "PAYPAL - NEGÓCIOS INVÁLIDOS - ID TXN: $txn_id | PAYPAL EMAIL: $payer_email | STATUS: $payment_status | Business: $business_email";
                gamecp_log(5, $user_name, $log_message);
            ...
            if ($payment_status == 'Reversed') {
                ...
                $log_message = "PAYPAL - <b>REVERSO</b> - TXN ID: $txn_id | PAYPAL EMAIL: $payer_email | Business: $business_email";
                gamecp_log(5, $user_name, $log_message);
                $ban_reason = "Auto - PayPal Reverso";
                $ban_period = 119988;
                ...
                if (!($insert_result = @mssql_query($insert_sql))) {
                    ...
                    gamecp_log(5, $user_name, "PAYPAL - <b>BANNED</b> - Proibição automática de reversão de pagamento", 1);
                }
            } elseif ($payment_status == 'Canceled_Reversal') {
                // Recommend this log as 'suspecious', level 5 log
                $log_message = "PAYPAL - <b>RESERVA CANCELADA</b> - TXN ID: $txn_id | PAYPAL EMAIL: $payer_email | Business: $business_email";
                gamecp_log(5, $user_name, $log_message);
            } else {
                // Recommend this log as 'suspecious', level 4 log
                $log_message = "PAYPAL - <b>IMCOMPLETO</b> - TXN ID: $txn_id | PAYPAL EMAIL: $payer_email | STATUS: $payment_status | Business: $business_email";
